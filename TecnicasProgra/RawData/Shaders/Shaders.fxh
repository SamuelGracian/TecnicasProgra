...
float4 PShader(PSIn input) : SV_Target0
{
    ...
    {
        return float4(0.02f, 0.08f, 0.20f, 1.0f); //defaut value for the color
    }
    ...
    float ndl = max(dot(normal, -lightDirection), 0.0f); // N dot L2
    float3 diffuse = ndl * color * shadow;

    float3 viewDirection = normalize(float3(0, 0, 1)); /// float is the camera direction
